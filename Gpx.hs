...
getPoint = atTag "trkpt" >>>
  proc x -> do
    ...
    returnA -< [Attr "position" (TB1 $ SGeo $ SPosition $ Position (read lon ,read lat ,read ele)),Attr "instant" (TB1 $ STimestamp $  fromJust $ fmap fst  $ strptime "%Y-%m-%dT%H:%M:%SZ" time )]
