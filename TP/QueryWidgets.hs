...
modifyTB  (EditTB m n ) =  fmap ( mapTB1 (\i -> maybe i id $ findTB1  (\k -> keyattr k == keyattr i) m ) ) (Just n)
...
addToList  (EditTB m n ) = (map (\e-> if  (e ==  n) then  mapTB1 (\i -> maybe i id $ findTB1 (\k -> keyattr k == keyattr i  ) m ) e  else e ))
