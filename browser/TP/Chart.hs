  ...
  ui$ transactionNoLog  (meta inf) $ do
      ...
      return $ fmap (\e ->
        let
          ...
          projf  r efield  = M.fromList [("value" ,ArrayTB1 $  attr <$> efield), ("title",txt (T.pack $  L.intercalate "," $ renderShowable <$> allKVRec' inf (tableMeta table) r)) , ("table",txt tname),("color" , txt $ T.pack $ "#" <> renderShowable color )] :: M.Map Text (FTB Showable)
