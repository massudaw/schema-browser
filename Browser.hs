...
setup args w = void $ do
  ...
  mapUIT body (traverse (\(conn,inf@(_,baseTables,_,schema,invSchema,graphP))-> do
    ...
    element body # set UI.children [span] )) evDB
...
classifyFK (Attr (KOptional (Primitive PDouble))) (Attr (KInterval (Primitive PDouble))) =  [Elem]
classifyFK (Attr (Primitive PDouble)) (Attr (KInterval (Primitive PDouble))) =  [Elem]
...
crudUITable conn inf pgs tb@(TB1 (KV (PK k d) a)) oldItems = do
  let
      tbCase td ix i@(FKT ifk _) = fkUITable conn inf pgs ((\(Just i)-> i) $ L.find (\(Path ifkp _ _) -> S.fromList (concat $ kattr <$> ifk) == ifkp) $ S.toList $ rawFKS table) (fmap ((!!ix) .td .unTB1) <$> oldItems) i
  ...
  (panelItems,evsa)<- processPanelTable conn inf (facts tb) table oldItems
  let commitedTB = tidings (facts tb) (unionWith const (facts tb <@ unions evsa) (rumors oldItems) )
  res <- mapM (pluginUI' conn inf commitedTB ) (filter (\(BoundedPlugin _ (t,_) _ ) -> t == tableName table) pgs)
  listBody <- UI.ul
...
processPanelTable conn inf attrsB table oldItemsi = do
...
fkUITable conn inf pgs (Path rl (FKJoinTable _  rel _ ) rr ) oldItems  tb@(FKT ifk tb1) = mdo
      ...
      let
        ...
        tdsel = fmap (\i -> FKT (zipWith (\ j i-> (,i) <$> j)  ifk . fmap snd  . findPK $ i ) i)  <$>  triding box
              ...
              if testShowable (<=14) (snd ano )
                ...
                MaybeT $ return  $ (\case {[] -> Nothing ; i -> Just i }) mods
                    ...
                    firemods <- C.lift $ do
                      ...
                      if not taxa_paga
                         then (if taxa_paga /= (not .snd $html) then updateMod  conn [(lookKey inf "fire_project" "taxa_paga" , SBoolean . not .snd $html)] inputs fire_project else return Nothing)
                         else return  Nothing
...
notaPrefeitura = BoundedPlugin "Nota Prefeitura" ("nota",staticP url) elem
  where
    ...
    url = proc t -> do
      ...
      act (fmap join  . traverse (\(i, (j, k,a)) -> prefeituraNota j k a i ) ) -< liftA2 (,) i r
    elem conn inf inputs = do
       ev <- mapT (dynPK url) inputs
       print<-printIFrame "creaFrame"
       frame<-iframe  # set UI.name "creaFrame" # set UI.id_ "creaFrame" # UI.sink (strAttr "srcdoc") (maybe "" id <$> facts ev) # UI.set UI.style [("width","100%"),("height","300px")]
           ...
           updateArtStatus  im it = do
              ...
              if null (i)
                 then return []
                   ...
                   v <- updateMod conn (i)  (fromJust $ fmap F.toList im) (lookTable inf "art")
...
executeStep  conn inf inputs (TBPlan t  ta steps) =  do
          ...
          case t of
            TInsert -> do
               traverse (\i -> do
                        mod <- insertMod conn (concat i) ta
            ...
            TUpdate -> do
               traverse (\i -> do
                        mod <- updateMod conn (concat i) inputs ta
                        traverse (logTableModification inf conn ) mod
                        return mod
                        ) (onlyJust k)
...
executeStep  conn inf inputs (SPFK t (Path  i (FKJoinTable ti s tj ) j) steps) = do
  ...
  traceShowId <$> joinTable (M.fromList $ swap <$> s) <$> case t of
       ...
       FKEUpdateFK -> do
         traverse (\i -> do
                    mod <- updateMod conn ( concat  i) inputs tj
                    traverse (logTableModification inf conn ) mod
...
--  preview $ cvLabeled (warshall graphP )
  startGUI (defaultConfig { tpStatic = Just "static", tpCustomHTML = Just "index.html"})  (setup args)
  print "Finish"
