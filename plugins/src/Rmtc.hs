...
checkOnibus inf = do
  ...
  fetchData  ref . fmap (RowPatch . fmap PatchRow)  $fmap (\i -> liftPatch inf "max_registro_onibus" <$> (Idex [ int (fromIntegral $ numeroOnibus i)], kvlistp [ PAttr "onibus" (patch$ int (fromIntegral $ numeroOnibus i)), PAttr "horario" (patch $ timestamp t),PAttr "situacao" (POpt $ Just $ patch $ Types.txt (situacao i)),PAttr "geocode" (POpt $ Just $ patch $  pos (Position (fst $ position i)(snd $ position i) 0))]))  dec
