...
joinRel2
  :: (Ord a ,Show a,G.Predicates (TBIndex a)) 
   => KVMetadata Key 
   -> [(Rel Key ,FTB a)]
   -> G.GiST (TBIndex a) (TBData Key a) 
   -> Maybe (FTB (TBData Key a))
joinRel2 tb ref table = recurse (relType' (relComp (fst <$> ref))) ref
  where
    recurse k ref 
      | L.any (isLeft.snd) ref || isKOptional k 
        = Just $ LeftTB1  $ join $ fmap (recurse  (unKOptional k)) (Tra.traverse (traverse unSOptional) ref )
      | L.any (isArray.snd) ref
        = let
            arr = justError ("no array"<> show ref )$ L.find (isArray .snd) ref
            arrayTB1 =  fmap (ArrayTB1 .  NonS.fromList ).nonEmpty
         in  join . fmap arrayTB1 . Tra.sequenceA   $ fmap (recurse k . (:L.filter (not .isArray .snd) ref)) 
    ...
    unKOptional (Primitive (KOptional :i) a) = unKOptional (Primitive i a)
    unKOptional (Primitive (KSerial :i) a) = unKOptional (Primitive i a)
    unKOptional i = i
